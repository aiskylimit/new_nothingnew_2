...
class TrainingArguments(TrainingArguments):
    ...
    scva_attention_layer: int = field(default=-1, metadata={"help": "Deprecated compatibility knob; SCVA now selects multiple layer pairs."})
    scva_n_layer_pairs: int = field(default=4, metadata={"help": "Number of decoder layer pairs used for SCVA."})
    scva_layer_low_pct: float = field(default=0.4, metadata={"help": "Lowest student decoder depth fraction sampled by SCVA."})
    scva_layer_high_pct: float = field(default=0.7, metadata={"help": "Highest student decoder depth fraction sampled by SCVA."})
    scva_sparse_attention: bool = field(default=True, metadata={"help": "Capture only response-to-vision attention at selected Qwen layers while the main forward uses SDPA."})
