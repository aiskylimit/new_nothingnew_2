#!/usr/bin/env bash
# Phase 3: gradient capture (--verify) for the Qwen3-4B track.
set -euo pipefail
...
OUTPUT_DIR="data/qwen3-4b/spectral"
...
for i in "${!GPUS[@]}"; do
  CUDA_VISIBLE_DEVICES="${GPUS[$i]}" python -u "${BASE_PATH}/src/gradient_capture.py" ${OPTS} --verify \
    ...
    > "logs/qwen3-4b-capture-shard${i}.log" 2>&1 &
  pids+=($!)
done
shard_fail=0
for i in "${!pids[@]}"; do
  wait "${pids[$i]}" || { echo "shard ${i} failed -- see logs/qwen3-4b-capture-shard${i}.log" >&2; shard_fail=1; }
done
...
echo "${CMD}"
CUDA_VISIBLE_DEVICES="${GPUS[0]}" ${CMD} 2>&1 | tee logs/qwen3-4b-capture.log
