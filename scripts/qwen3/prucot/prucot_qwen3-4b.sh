#!/usr/bin/env bash
# Phase 5: masked SFT -- PRU-COT baseline, Qwen3-4B track (LoRA, same setup as spectral).
set -euo pipefail
...
DATA_PATH="${BASE_PATH}/data/qwen3-4b/train-prucot.jsonl"
OUTPUT_DIR="${BASE_PATH}/checkpoints/prucot-qwen3-4b"
EPOCHS=3
LR=5.0e-5
MIN_LR=1.0e-5
WARMUP_RATIO=0.1
...
echo "${CMD}"
${CMD} 2>&1 | tee "${BASE_PATH}/logs/prucot-qwen3-4b.log"
