...
LR=10
FILLER_TOKEN="."
# Empty = keep all; set to drop longer records (paper-style). Must match prune_qwen3-1.7b.sh's value.
...
for i in "${!GPUS[@]}"; do
  CUDA_VISIBLE_DEVICES="${GPUS[$i]}" python -u "${BASE_PATH}/src/prucot_weight.py" ${OPTS} \
    ...
    > "logs/qwen3-1.7b-prucot-weight-shard${i}.log" 2>&1 &
  pids+=($!)
done
shard_fail=0
for i in "${!pids[@]}"; do
  wait "${pids[$i]}" || { echo "shard ${i} failed -- see logs/qwen3-1.7b-prucot-weight-shard${i}.log" >&2; shard_fail=1; }
done
...
echo "${CMD}"
CUDA_VISIBLE_DEVICES="${GPUS[0]}" ${CMD} 2>&1 | tee logs/qwen3-1.7b-prucot-weight.log
