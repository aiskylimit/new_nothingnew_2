#!/usr/bin/env bash
# Phase 6: eval a DeepSeek-R1-Distill-Qwen-7B LoRA adapter with vLLM, Pass@1. Default = spectral ckpt;
# pass a checkpoint path + tag to eval another (e.g. the vanilla baseline).
#   ./scripts/eval/eval_r1-qwen-7b.sh
#   ./scripts/eval/eval_r1-qwen-7b.sh checkpoints/vanilla-r1-qwen-7b vanilla-r1-qwen-7b
set -euo pipefail
...
MODEL="${BASE_PATH}/checkpoints/spectral-r1-qwen-7b"
BASE_MODEL="${LOCAL_MODELS_ROOT}/DeepSeek-R1-Distill-Qwen-7B"
TAG="spectral-r1-qwen-7b"
[[ -n "${1:-}" ]] && MODEL="$1"
[[ -n "${2:-}" ]] && TAG="$2"
