"""RLSD (RLVR with Self-Distillation, arXiv:2604.03128) on allenai/Olmo-3-7B-Think,
copied from offline_rlsd_sdpo_b200/scripts/run_rlsd_experiment_8b.py (same
effective rollout budget as that 8B script - group_size=2, questions_per_step=2 -
since OLMo-3-7B is closer in scale to Qwen3-8B than 4B; not separately
verified for OLMo's own memory profile). Same LoRA config (r=64, alpha=128,
SAME target_modules - verified against transformers' real modeling_olmo3.py
...
assumed), same 100 training steps / lr=5e-6 / max_grad_norm=0.1 / gradient_checkpointing,
...
with or without it). Its `add_generation_prompt=True` output already ends
on an OPENED `<think>` tag, so the model always starts reasoning by default
(matches the teacher's own always-thinking behavior here, unmodified - RLSD's
teacher prefix is built with `enable_thinking=True` regardless of model).
The STUDENT rollout, however, must be non-thinking - CONFIG["empty_think_suffix"]
= "\\n\\n</think>\\n\\n" is threaded through tropic.data.build_student_prompt
(via tropic.verifier_data.load_opsd_math_examples_with_answer) so the
auto-opened `<think>` is immediately closed empty (the community "empty
think prefill" trick documented for DeepSeek-R1-style models - see
huggingface.co/deepseek-ai/DeepSeek-R1-Distill-Qwen-14B/discussions/11).
UNVERIFIED END-TO-END: only confirmed the rendered PROMPT ends in
`<think>\\n\\n</think>\\n\\n` - NOT confirmed Olmo-3-7B-Think actually skips
reasoning after seeing this the way DeepSeek-R1 does. Inspect real student
rollout text early in a run before trusting this.
...
GPUs per checkpoint step (identical pattern to run_full_experiment_4b.py):
    CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. python scripts/run_rlsd_experiment_olmo7b.py \\
        --model-path /path/to/local/Olmo-3-7B-Think --seed 0 --output-dir results_rlsd_olmo7b_seed0 --skip-eval
    # then, per checkpoint step x per benchmark, shard across 4 GPUs:
    CUDA_VISIBLE_DEVICES=$i PYTHONPATH=. python scripts/run_rlsd_experiment_olmo7b.py \\
        --model-path /path/to/local/Olmo-3-7B-Think --output-dir results_rlsd_olmo7b_seed0 --skip-train \\
        --checkpoint-path results_rlsd_olmo7b_seed0/rlsd_checkpoint_step100 \\
...
parser.add_argument("--output-dir", default="results_rlsd_olmo7b",
