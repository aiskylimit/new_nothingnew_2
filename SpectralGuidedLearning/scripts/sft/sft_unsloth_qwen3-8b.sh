...
read -ra GPUS <<< "${GPUS:-0}"
if [[ "${#GPUS[@]}" -ne 1 ]]; then
  echo "sft_unsloth_qwen3-8b.sh: single-GPU only. Got GPUS='${GPUS[*]}'." >&2
  exit 2
fi
...
OUTPUT_DIR="${BASE_PATH}/checkpoints/vanilla-unsloth-qwen3-8b"
EPOCHS=3
LR=5.0e-5
MIN_LR=1.0e-5
WARMUP_RATIO=0.1
...
echo "${CMD}"
${CMD} 2>&1 | tee "${BASE_PATH}/logs/vanilla-unsloth-qwen3-8b.log"
