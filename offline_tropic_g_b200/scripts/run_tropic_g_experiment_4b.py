...
scripts/run_rlsd_experiment_4b.py / run_sdpo_experiment_4b.py): same base
model (Qwen/Qwen3-4B, overridable via --model-path for a local offline
...
run_rlsd_experiment_4b.py's own usage pattern exactly):
    CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. python scripts/run_tropic_g_experiment_4b.py \\
        --model-path /path/to/local/Qwen3-4B --seed 0 --output-dir results_tropic_g_4b --skip-eval \\
        --use-vllm-rollout --vllm-gpu-ids 3 --vllm-base-port 8100
    CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. python scripts/run_tropic_g_experiment_4b.py \\
        --model-path /path/to/local/Qwen3-4B --skip-train \\
        --checkpoint-path results_tropic_g_4b/tropic_g_checkpoint_step100 \\
        --output-dir results_tropic_g_4b --eval-engine vllm --vllm-gpu-ids 1,2,3 --vllm-base-port 8100
"""
import argparse
import json
import re
...
parser.add_argument("--output-dir", default="results_tropic_g_4b",
