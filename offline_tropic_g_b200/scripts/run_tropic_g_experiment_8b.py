...
as an offline-deployment baseline, run under the SAME experimental setting
this offline package already uses for RLSD/SDPO (see
scripts/run_rlsd_experiment_8b.py / run_sdpo_experiment_8b.py): same base
model (Qwen/Qwen3-8B, overridable via --model-path for a local offline
directory), same LoRA config (r=64, alpha=128, same target modules), same
lr=5e-6/max_grad_norm=0.1/gradient_checkpointing, same checkpoint cadence
...
DELTAS from that shared setting, exactly as agreed with the user before this
file was written:
  ...
  - FULL VOCABULARY loss (no top-K sparsification): passed as
    `top_k=200000` in CONFIG["tropic_g"] rather than a code change -
    `tropic.primitives.build_shared_topk_mask` already does
    `top_k = min(top_k, vocab_size)` internally, and Qwen3's vocab is
    ~152K, so a top_k larger than that collapses to the full vocabulary
    with ZERO changes to tropic/loss.py's existing sparsification
    primitives (confirmed via that function's own source before writing
    this file). Chunking (an alternative OOM-mitigation approach discussed
    for full-vocab losses) was deliberately NOT implemented - B200's much
    larger VRAM than the A100s this concern was originally raised for makes
    "try full vocab first, add chunking only if it OOMs" the agreed
    approach; if it OOMs, add chunking here rather than reverting to top-K.
  - effective_batch_size=4 (this offline package's own choice, matching
    ...
    script (run_tropic_g_experiment_4b.py) uses effective_batch_size=16,
...
run_rlsd_experiment_8b.py's own usage pattern exactly):
    CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. python scripts/run_tropic_g_experiment_8b.py \\
        --model-path /path/to/local/Qwen3-8B --seed 0 --output-dir results_tropic_g_8b --skip-eval \\
        --use-vllm-rollout --vllm-gpu-ids 3 --vllm-base-port 8100
    CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. python scripts/run_tropic_g_experiment_8b.py \\
        --model-path /path/to/local/Qwen3-8B --skip-train \\
        --checkpoint-path results_tropic_g_8b/tropic_g_checkpoint_step100 \\
        --output-dir results_tropic_g_8b --eval-engine vllm --vllm-gpu-ids 1,2,3 --vllm-base-port 8100
"""
import argparse
import json
import re
...
parser.add_argument("--output-dir", default="results_tropic_g_8b",
